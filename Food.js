...
const Body = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setRestaurantList(data.categories);
  ...
  useEffect(() => {
    // try{
    //   fetch('https://www.themealdb.com/api/json/v1/1/categories.php').then((res)=> res.json()).then
    // ((data)=>setRestaurantList(data));
    // }
    // catch(e){
    //   console.log(e);
    // }
    ...
    fetchData();
    // console.log('restaur',restaurantList);
  },[]);
  return (
    <div className="restaurant-list">
      ...
      {restaurantList.map(restaurant => (
        <div key={restaurant.idCategory}>
          <img src={restaurant?.strCategoryThumb} />

          <h2>name : {restaurant?.strCategory}</h2>
          <p>description {restaurant?.strCategoryDescription}</p>
        </div>
        //  <RestaurantCard key={restaurant?.categories?.idCategory} {...restaurant?.categories} />
      ))}
    </div>
  );
};
